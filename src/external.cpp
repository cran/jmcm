...
Rcpp::List mcd_estimation(arma::vec m,
	...
	double f = mcd(x);
	arma::vec grad; mcd.Gradient(x, grad);
	...
	for (int iter = 0; iter != kIterMax; ++iter){
	    ...
	    if (test < kTolX) {
		if(debug)
		    Rcpp::Rcout << "Test for convergence on Delta x: converged." << std::endl;
		break;
	    }
	    ...
	    for(int i = 0; i != n_pars; ++i)
		{
		    double temp = std::abs(grad(i))
			* std::max(std::abs(x(i)), 1.0) / den;
		    if (temp > test) test = temp;
		}
	    if(debug2) Rcpp::Rcout << "test2 = " << test << std::endl;
	    if (test < grad_tol) {
		if(debug)
		    Rcpp::Rcout << "Test for convergence on zero gradient: converged." << std::endl;
		break;
	    }
...
Rcpp::List acd_estimation(arma::vec m,
	...
	double f = acd(x);
	arma::vec grad; acd.Gradient(x, grad);
...
Rcpp::List hpc_estimation(arma::vec m,
	...
	double f = hpc(x);
	arma::vec grad; hpc.Gradient(x, grad);
